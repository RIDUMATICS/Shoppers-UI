...
export default {
  setup() {
    ...
    watch(
      () => data.verifyPassword,
      (value) => {
        if (value === data.password) data.isConfirmPassword = true;
        else data.isConfirmPassword = false;
      }
    );
    ...
    watch(
      () => data.password,
      (value) => {
        if (value === data.verifyPassword) data.isConfirmPassword = true;
        else data.isConfirmPassword = false;
      }
    );
    ...
    async function registerNewUser() {
      if (data.password !== data.verifyPassword) {
        store.commit('popupAlert', {
          head: 'Unable to register user',
          body: 'password and confirm password does not match',
          status: 'error',
        });
      } else {
        try {
          ...
          if (error.status === 409) {
            ...
            store.commit('popupAlert', {
              head: 'Unable to register user',
              body: "Something happend, but i'm not telling you what",
              status: 'error',
            });
          }
        }
...
.sigin_part_text {
  background-image: linear-gradient(
    ...
    url('../assets/images/slide-07.jpg');
...
.sigin_part_form input[type='password'] {
...
.sigin_part_form input[type='password']:focus {
