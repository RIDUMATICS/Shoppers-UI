<template>
  <main>
    <section class="section-padding container" v-if="order">
      <div class="confirmation-title"><p>Thank you. Your order has been received.</p></div>
        ...
        <div>
          <div>
            <div class="single-confirmation-details">
              <h4>Order Info</h4>
              <ul>
                <li><p>Order No </p><span>: {{ order.id }}</span></li>
                <li><p>Date </p><span>: {{ order.createdAt }}</span></li>
                <li><p>Delivery </p><span>: {{ order.isDelivered ? '':'Not' }} Delivered</span></li>
                <li><p>Payment </p><span>: {{ order.isPaid ? '':'Not' }} Paid</span></li>
              </ul>
            </div>
            <div class="single-confirmation-details">
              <h4>Shipping Address</h4>
              <ul>
                <li><p>Street </p><span>: {{ order.shipping.address }}</span></li>
                <li><p>City </p><span>: {{ order.shipping.city }}</span></li>
                <li><p>State </p><span>: {{ order.shipping.state }}</span></li>
                <li><p>Postal code </p><span>: {{ order.shipping.postalCode }}</span></li>
              </ul>
            </div>
          </div>
          <div class="single-confirmation-details">
            <h4>Order Summary</h4>
            <ul>
              <li><p>Items </p><span>: {{ format(order.itemsPrice) }}</span></li>
              <li><p>Shipping </p><span>: {{ format(order.shippingPrice )}}</span></li>
              <li class="total-price"><p>Total </p><span>: {{ format(order.totalPrice)}}</span></li>
            </ul>
            <Paystack
          ...
          <table>
            ...
            <tbody>
              <tr v-for="orderItem in order.orderItems" :key="orderItem.id">
                <td colspan="2">{{ orderItem.name }}</td>
...
export default {
  setup(){
    ...
    const data = reactive({
      ...
      PUBLIC_KEY: "pk_test_87a63b870e30ae22db2679fbfcd1a32e576dd118",
    })
    
    async function processPayment (e) {
      try {
        await store.dispatch('payOrder', e.reference)
      } catch (error) {
        console.log(error);
      }
    }
