<template>
  <main>
    <h1>
      {{ pathName == 'createProduct' ? 'Create Product' : 'Edit Product' }}
    </h1>
    <form>
      ...
      <div required class="form-group" v-if="pathName == 'createProduct'">
      ...
      <div class="form-group">
        ...
        <input required type="number" min="1" id="price" v-model="price" />
      </div>
      <div class="form-group">
...
export default {
  setup() {
    ...
    async function createProduct() {
      try {
        if (!data.checkedProductFors.length) {
          store.commit('popupAlert', {
            head: 'Unable to create',
            body: 'Please fill all product details',
            status: 'error',
          });
        } else {
    ...
    onMounted(async () => {
      try {
        if (route.name == 'editProduct') {
          ...
          const { productId } = route.params;
          const { product } = await store.dispatch('fetchProductById', {
            productId,
          });
