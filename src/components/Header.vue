      ...
      <div class="navigation container">
        <div class="logo">
          <router-link to="/"
            ><h1>
              Shoppers
              <span
                style="display:inline-block; background-color: #fc7c7c; width: 7px; height: 7px; border-radius: 50%; margin-left: -3px; margin-bottom: 0;"
              ></span></h1
          ></router-link>
        </div>
        <div class="menu" :class="{ show: showMenu }">
          ...
          <ul class="nav-list" @click.passive="handleClick">
            <li class="nav-item">
              <router-link to="/" class="nav-link" data-type="menu"
                >Home</router-link
              >
            </li>
            <li class="nav-item">
              <router-link to="/shop/1" class="nav-link" data-type="menu"
                >Shop</router-link
              >
            </li>
            <li class="nav-item">
            ...
            <li class="nav-item" v-show="!isAuthenticated">
              <router-link to="/login" class="nav-link" data-type="menu"
            ...
            <li class="nav-item cart">
              <router-link to="/cart" class="nav-link"
                ...
                <span class="cart-num">{{ carts.length }}</span>
              </router-link>
            </li>
          </ul>
        </div>
        ...
        <router-link to="/cart" class="cart-icon">
          <font-awesome-icon :icon="faShoppingBag" />
          <span class="cart-num-mobile">{{ carts.length }}</span>
        </router-link>
...
export default {
  setup() {
    ...
    function handleShowMenu(isShow) {
      if (isShow) document.body.classList.add('show');
      else document.body.classList.remove('show');
